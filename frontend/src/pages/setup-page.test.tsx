...
it("asks for user-supplied data-source credentials during first-run setup", async () => {
  ...
  expect(screen.getByLabelText("FRED API key")).toHaveAttribute("type", "password")
  expect(screen.getByLabelText("SEC fair-access identity")).toBeInTheDocument()
  await user.type(screen.getByLabelText("Tiingo token"), "tiingo-test")
  await user.type(screen.getByLabelText("FRED API key"), "fred-test")
  await user.type(screen.getByLabelText("SEC fair-access identity"), "Test User test@example.com")
  await user.click(screen.getByRole("button", { name: "Save and continue" }))
  await waitFor(() => expect(saveTiingoMock).toHaveBeenCalledWith("tiingo-test"))
  expect(saveFredMock).toHaveBeenCalledWith("fred-test")
  expect(saveEdgarMock).toHaveBeenCalledWith("Test User test@example.com")
})
