...
export function SetupPage() {
  ...
  const saveDataSources = async () => {
    ...
    try {
      ...
      if (!runtime?.data.fred_configured) await api.saveFred(fredKey)
      if (!runtime?.data.sec_configured) await api.saveEdgarIdentity(edgarIdentity)
      setToken(""); setFredKey(""); setEdgarIdentity(""); await load(); setStep(3)
    }
  ...
  const dataSourcesConfigured = Boolean(runtime?.data.tiingo_configured && runtime?.data.fred_configured && runtime?.data.sec_configured)
  const dataSourceInputsComplete = Boolean(
    (runtime?.data.tiingo_configured || token.trim()) &&
    (runtime?.data.fred_configured || fredKey.trim()) &&
    (runtime?.data.sec_configured || edgarIdentity.trim()),
  )
  ...
  return <main className="grid min-h-screen place-items-center bg-muted/35 p-4"><Card className="w-full max-w-2xl shadow-lg"><CardContent className="p-7 sm:p-9">
    ...
    {step === 1 && <div className="space-y-4"><h2 className="text-lg font-semibold">Your research stays local</h2><p className="text-sm text-muted-foreground">Financial data and your data-source credentials are stored on this computer. Initial setup downloads the supported research dataset using your own SEC identity, FRED API key, and Tiingo token.</p><Button onClick={() => setStep(dataSourcesConfigured ? 3 : 2)}>Continue</Button></div>}
    {step === 2 && <div className="space-y-4"><div><h2 className="text-lg font-semibold">Connect data sources</h2><p className="text-sm text-muted-foreground">Enter only the values that are not already configured. They are saved in Reasonframe's owner-only local application-data directory and are never included in the application bundle.</p></div>
      {!runtime?.data.tiingo_configured && <label className="block text-sm font-medium">Tiingo token<input type="password" autoComplete="off" aria-label="Tiingo token" value={token} onChange={(event) => setToken(event.target.value)} className="mt-1 h-10 w-full rounded-md border bg-background px-3 text-sm" placeholder="Tiingo API token"/></label>}
      {!runtime?.data.fred_configured && <label className="block text-sm font-medium">FRED API key<input type="password" autoComplete="off" aria-label="FRED API key" value={fredKey} onChange={(event) => setFredKey(event.target.value)} className="mt-1 h-10 w-full rounded-md border bg-background px-3 text-sm" placeholder="32-character FRED API key"/></label>}
      {!runtime?.data.sec_configured && <label className="block text-sm font-medium">SEC fair-access identity<input type="text" autoComplete="name" aria-label="SEC fair-access identity" value={edgarIdentity} onChange={(event) => setEdgarIdentity(event.target.value)} className="mt-1 h-10 w-full rounded-md border bg-background px-3 text-sm" placeholder="Your Name you@example.com"/><span className="mt-1 block text-xs font-normal text-muted-foreground">SEC requests must identify you with a name and contact email.</span></label>}
      <Button onClick={saveDataSources} disabled={busy || !dataSourceInputsComplete}>{busy ? "Saving…" : "Save and continue"}</Button>
    </div>}
